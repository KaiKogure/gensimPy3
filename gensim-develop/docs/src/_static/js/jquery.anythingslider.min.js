...
(function (d, l, k) {
    d.anythingSlider = function (j, n) {
        ...
        a.init = function () {
            ...
            m = (b.mode || "h").toLowerCase().match(/(h|v|f)/);
            m = b.vertical ? "v" : (m || ["h"])[0];
            b.mode = "v" === m ? "vertical" : "f" === m ? "fade" : "horizontal";
            "f" === m && (b.showMultiple = 1, b.infiniteSlides = !1);
        ...
        a.buildNavigation = function () {
            if (b.buildNavigation && 1 < a.pages) {
                ...
                }, c).get(), a.navLeft = a.currentPage, a.$nav.width(a.navWidth(1, a.pages + 1) + 25), a.$controls.find(".anythingNavWindow").width(a.navWidth(1, b.navigationSize + 1)).end().find(".prev,.next").bind(b.clickControls, function (c) {
        ...
        a.checkResize = function (b) {
            var f = !(!k.hidden && !k.webkitHidden && !k.mozHidden && !k.msHidden);
        ...
        a.setDimensions = function () {
            a.$wrapper.find(".anythingWindow, .anythingBase, .panel").andSelf().css({width: "", height: ""});
            a.width = a.$el.width();
            a.height = a.$el.height();
            a.outerPad = [a.$wrapper.innerWidth() - a.$wrapper.width(), a.$wrapper.innerHeight() - a.$wrapper.height()];
            var c, f, e, g, h = 0, j = {width: "100%", height: "100%"}, k = 1 < b.showMultiple ? a.width || a.$window.width() / b.showMultiple : a.$window.width(), l = a.$win.width();
            b.expand && (a.lastDim = [a.$outer.width(), a.$outer.height()], c = a.lastDim[0] - a.outerPad[0], a.height = f = a.lastDim[1] - a.outerPad[1], a.$wrapper.add(a.$window).css({width: c, height: f}), a.width = k = 1 < b.showMultiple ? c / b.showMultiple : c, a.$items.css({width: k, height: f}));
            a.$items.each(function (m) {
                g = d(this);
                e = g.children();
                b.resizeContents ? (c = a.width, f = a.height, g.css({width: c, height: f}), e.length && ("EMBED" === e[0].tagName && e.attr(j), "OBJECT" === e[0].tagName && e.find("embed").attr(j), 1 === e.length && e.css(j))) : (c = g.width() || a.width, 1 === e.length && c >= l && (c = e.width() >= l ? k : e.width(), e.css("max-width", c)), g.css({width: c, height: ""}), f = 1 === e.length ? e.outerHeight(!0) : g.height(), f <= a.outerPad[1] && (f = a.height), g.css("height", f));
                a.panelSize[m] = [c, f, h];
        ...
        a.gotoPage = function (c, f, e, g) {
            ...
            if (!(1 >= a.pages) && (a.$lastPage = a.$currentPage, "number" !== typeof c && (c = parseInt(c, 10) || b.startPanel, a.setCurrentPage(c)), !f || !b.isVideoPlaying(a)))a.exactPage = c, c > a.pages + 1 - a.adj && (c = !b.infiniteSlides && !b.stopAtEnd ? 1 : a.pages), c < a.adj && (c = !b.infiniteSlides && !b.stopAtEnd ? a.pages : 1), b.infiniteSlides || (a.exactPage = c), a.currentPage = c > a.pages ? a.pages : 1 > c ? 1 : a.currentPage, a.$currentPage = a.$items.eq(a.currentPage - a.adj), a.targetPage = 0 === c ? a.pages : c > a.pages ? 1 : c, a.$targetPage = a.$items.eq(a.targetPage - a.adj), g = "undefined" !== typeof g ? g : b.animationTime, 0 <= g && a.$el.trigger("slide_init", a), 0 < g && a.slideControls(!0), b.buildNavigation && a.setNavigation(a.targetPage), !0 !== f && (f = !1), (!f || b.stopAtEnd && c === a.pages) && a.startStop(!1), 0 <= g && a.$el.trigger("slide_begin", a), setTimeout(function (d) {
                ...
                b.resizeContents || (f = a.getDim(c), d = {}, a.$wrapper.width() !== f[0] && (d.width = f[0] || a.width, h = !1), a.$wrapper.height() !== f[1] && (d.height = f[1] || a.height, h = !1), h || a.$wrapper.filter(":not(:animated)").animate(d, {queue: !1, duration: 0 > g ? 0 : g, easing: b.easing}));
                "fade" === b.mode ? a.$lastPage[0] !== a.$targetPage[0] ? (a.fadeIt(a.$lastPage, 0, g), a.fadeIt(a.$targetPage, 1, g, function () {
                    a.endAnimation(c, e, g)
                })) : a.endAnimation(c, e, g) : (d = {}, d[a.dir] = -a.panelSize[b.infiniteSlides && 1 < a.pages ? c : c - 1][2], a.$el.filter(":not(:animated)").animate(d, {queue: !1, duration: 0 > g ? 0 : g, easing: b.easing, complete: function () {
                    a.endAnimation(c, e, g)
                }}))
        ...
        a.startStop = function (c, d) {
            ...
            c ? (a.clearTimer(!0), a.timer = l.setInterval(function () {
                k.hidden || k.webkitHidden || k.mozHidden || k.msHidden ? b.autoPlayLocked || a.startStop() : b.isVideoPlaying(a) ? b.resumeOnVideoEnd || a.startStop() : a.goForward(!0)
    ...
    d.fn.anythingSlider = function (j, k) {
        return this.each(function () {
            var a, b = d(this).data("AnythingSlider");
            (typeof j).match("object|undefined") ? b ? b.updateSlider() : new d.anythingSlider(this, j) : /\d/.test(j) && !isNaN(j) && b ? (a = "number" === typeof j ? j : parseInt(d.trim(j), 10), 1 <= a && a <= b.pages && b.gotoPage(a, !1, k)) : /^[#|.]/.test(j) && d(j).length && b.gotoPage(j, !1, k)
        }, b)
    }
