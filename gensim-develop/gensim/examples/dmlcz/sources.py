    ...
    def parseDmlMeta(cls, xmlfile):
        ...
        for line in xml:
            if line.find('</article>') >= 0: # end of <article>, we're done
                break
            p = re.search(PAT_TAG,
                          line) # HAX assumes one element = one line; proper xml parsing probably better... but who cares
            if p:
                name, cont = p.groups()
                name = name.split()[0]
...
class DmlCzSource(DmlSource):
    ...
    def isArticle(self, path):
        ...
        if not (os.path.exists(os.path.join(path, 'fulltext.txt')) or os.path.exists(
                os.path.join(path, 'fulltext-dspace.txt'))):
            logger.info('missing fulltext in %s' % path)
            return False
...
class ArxmlivSource(ArticleSource):
    ...
    class ArxmlivContentHandler(xml.sax.handler.ContentHandler):
        ...
        def characters(self, text):
            ...
            if self.path[-1] == 'p':
                tokens = [token.encode('utf8') for token in utils.tokenize(text, errors='ignore') if
                          not token.isdigit()]
