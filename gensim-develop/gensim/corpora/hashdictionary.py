...
class HashDictionary(utils.SaveLoad, collections.MutableMapping):
    ...
    def add_documents(self, documents):
        ...
        for docno, document in enumerate(documents):
            ...
            _ = self.doc2bow(document,
                             allow_update=True) # ignore the result, here we only care about updating token ids
    ...
    def filter_extremes(self, no_below=5, no_above=0.5, keep_n=100000):
        ...
        **Note:** since HashDictionary's id range is fixed and doesn't depend on
        ...
        self.id2token = dict((tokenid, set(token for token in tokens if token in self.dfs_debug)) for tokenid, tokens in
                             self.id2token.items())
