...
class IndexedCorpus(interfaces.CorpusABC):
    ...
    @classmethod
    def serialize(cls, fname, corpus, id2word=None, index_fname=None, progress_cnt=None, labels=None):
        """
        ...
        * the `docbyoffset(offset)` method, which returns a document
        ...
        if progress_cnt is not None:
            if labels is not None:
                offsets = cls.save_corpus(fname, corpus, id2word, labels=labels, progress_cnt=progress_cnt)
            else:
                offsets = cls.save_corpus(fname, corpus, id2word, progress_cnt=progress_cnt)
        else:
            if labels is not None:
                offsets = cls.save_corpus(fname, corpus, id2word, labels=labels)
            else:
                offsets = cls.save_corpus(fname, corpus, id2word)
