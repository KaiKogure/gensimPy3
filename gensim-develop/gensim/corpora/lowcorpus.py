...
class LowCorpus(IndexedCorpus):
    ...
    def __init__(self, fname, id2word=None, line2words=split_on_space):
        ...
        self.num_docs = int(open(
            fname).readline()) # the first line in input data is the number of documents (integer). throws exception on bad input.
        ...
        if not id2word:
            ...
            self.id2word = dict(
                zip(range(len(all_terms)), all_terms)) # build a mapping of word id(int) -> word (string)
        else:
    ...
    def line2doc(self, line):
        ...
        if self.use_wordids:
            ...
            uniq_words = set(words).intersection(iter(self.word2id.keys()))
            ...
            doc = list(zip(list(map(self.word2id.get, use_words)), list(map(words.count,
                                                                            use_words)))) # using list.count is suboptimal but speed of this whole function is irrelevant
        else:
            uniq_words = set(words)
            # construct a list of (word, wordFrequency) 2-tuples
            doc = list(zip(uniq_words, list(
                map(words.count, uniq_words)))) # using list.count is suboptimal but that's irrelevant at this point
