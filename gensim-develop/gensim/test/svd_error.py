...
if __name__ == '__main__':
    ...
    for chunk in gensim.utils.grouper(corpus, chunksize=5000):
        ...
        chunk = gensim.matutils.corpus2csc(chunk, num_nnz=num_nnz, num_terms=m, num_docs=len(chunk),
                                           dtype=numpy.float32)
        chunk = chunk * chunk.T
        chunk = chunk.toarray()
        aat += chunk
    ...
    for factors in FACTORS:
        err = -numpy.dot(spectrum_u[:, :factors],
                         numpy.dot(numpy.diag(spectrum_s[:factors]), spectrum_u[:, :factors].T))
        err += aat
        ...
        for power_iters in POWER_ITERS:
            for chunksize in CHUNKSIZE:
                ...
                del model
                print(
                    ("incremental SVD for %i factors, %i power iterations, chunksize %i took %s s (spectrum %f .. %f)" %
